package com.example.pogun.service.payment;

import com.apple.itunes.storekit.client.APIException;
import com.apple.itunes.storekit.client.AppStoreServerAPIClient;
import com.apple.itunes.storekit.model.Environment;
import com.apple.itunes.storekit.model.JWSTransactionDecodedPayload;
import com.apple.itunes.storekit.model.ResponseBodyV2DecodedPayload;
import com.apple.itunes.storekit.model.TransactionInfoResponse;
import com.apple.itunes.storekit.verification.SignedDataVerifier;
import com.apple.itunes.storekit.verification.VerificationException;
import com.example.pogun.config.payment.PaymentProperties;
import com.example.pogun.dto.common.ApiResponse.ApiException;
import org.springframework.stereotype.Component;

import java.io.ByteArrayInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.nio.charset.StandardCharsets;
import java.util.Base64;
import java.util.List;
import java.util.Set;
import java.util.stream.Collectors;

@Component
public class AppleStoreKitClient {

    private static final List<String> APPLE_ROOT_CERTIFICATES_BASE64 = List.of(
            "",
            "",
            ""
    );

    public String getSignedTransactionInfo(PaymentProperties.Apple apple, String transactionId) {
        try {
            TransactionInfoResponse response = apiClient(apple).getTransactionInfo(transactionId);
            return response == null ? null : response.getSignedTransactionInfo();
        } catch (APIException | IOException e) {
            throw ApiException.badRequest("APPLE_TRANSACTION_VERIFICATION_FAILED", "Apple 거래 검증에 실패했습니다.");
        }
    }

    public JWSTransactionDecodedPayload verifyAndDecodeTransaction(PaymentProperties.Apple apple, String signedTransactionInfo) {
        try {
            return signedDataVerifier(apple).verifyAndDecodeTransaction(signedTransactionInfo);
        } catch (VerificationException | IllegalArgumentException e) {
            throw ApiException.badRequest("INVALID_SIGNED_TRANSACTION_INFO", "Apple 거래 서명을 검증할 수 없습니다.");
        }
    }

    public ResponseBodyV2DecodedPayload verifyAndDecodeNotification(PaymentProperties.Apple apple, String signedPayload) {
        try {
            return signedDataVerifier(apple).verifyAndDecodeNotification(signedPayload);
        } catch (VerificationException | IllegalArgumentException e) {
            throw ApiException.badRequest("INVALID_APP_STORE_NOTIFICATION", "Apple 서버 알림 서명을 검증할 수 없습니다.");
        }
    }

    private AppStoreServerAPIClient apiClient(PaymentProperties.Apple apple) {
        return new AppStoreServerAPIClient(
                decodedPrivateKey(apple),
                apple.getKeyId(),
                apple.getIssuerId(),
                apple.getBundleId(),
                environment(apple)
        );
    }

    private SignedDataVerifier signedDataVerifier(PaymentProperties.Apple apple) {
        return new SignedDataVerifier(rootCertificates(), apple.getBundleId(), apple.getAppAppleId(), environment(apple), true);
    }

    private Set<InputStream> rootCertificates() {
        return APPLE_ROOT_CERTIFICATES_BASE64.stream()
                .map(encoded -> new ByteArrayInputStream(Base64.getDecoder().decode(encoded)))
                .collect(Collectors.toUnmodifiableSet());
    }

    private String decodedPrivateKey(PaymentProperties.Apple apple) {
        return new String(Base64.getDecoder().decode(apple.getPrivateKeyBase64()), StandardCharsets.UTF_8);
    }

    private Environment environment(PaymentProperties.Apple apple) {
        return "production".equalsIgnoreCase(apple.getEnvironment())
                ? Environment.PRODUCTION
                : Environment.SANDBOX;
    }
}
