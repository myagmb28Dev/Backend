...
/**
 * HTTP/WebSocket 진입점을 담당하는 AuthController입니다.
 */
@Slf4j
@RestController
...
public class AuthController {
    ...
    @PostMapping("/login")
    @Operation(summary = "소셜 로그인", description = "Firebase ID Token을 사용하여 소셜 로그인을 수행합니다. 없으면 자동 가입합니다.")
    public ResponseEntity<ApiResponse<AuthResponse>> firebaseSignIn(@Valid @RequestBody LoginRequest request,
        ...
        try {
            // Firebase 인증은 완료되었고, 신규 사용자는 정식 회원 대기 상태로 분리된다.
    ...
    @Operation(summary = "관리자 로그인", description = "구형 관리자 로그인 경로입니다. 관리자 콘솔은 /api/admin/auth/login PassKey 흐름을 사용합니다.")
    public ResponseEntity<ApiResponse<AuthResponse>> adminSignIn(@Valid @RequestBody LoginRequest request,
                                                                 HttpServletRequest httpRequest) {
        throw ApiException.forbidden(
                "LEGACY_ADMIN_LOGIN_DISABLED",
                "구형 관리자 로그인 경로는 비활성화되었습니다. /api/admin/auth/login PassKey 인증 흐름을 사용해주세요."
        );
    }
    ...
    @Operation(
            ...
            description = """
                    ...
                    프론트는 data.idToken을 즉시 현재 Firebase ID Token으로 교체하고, data.refreshToken은 로컬 세션 저장소에 갱신해야 합니다.
    ...
    @Operation(summary = "온보딩 완료", description = "소셜 인증 후 위치 등 정보를 받아 정식 회원가입을 완료합니다.")
    ...
    @Operation(summary = "온보딩 지도 설정 조회", description = "온보딩 위치 선택 화면에서 사용할 카카오 지도 설정 상태를 조회합니다.")
    ...
    @Operation(summary = "소셜 계정 연동 해제", description = "지정한 소셜 제공자의 연동을 해제합니다. 예: GOOGLE, APPLE")
    public ResponseEntity<ApiResponse<SocialUnlinkResponse>> unlinkSocial(@PathVariable String provider) {
        // provider 문자열은 서비스에서 정규화하고 마지막 연동 차단인지까지 검증한다.
