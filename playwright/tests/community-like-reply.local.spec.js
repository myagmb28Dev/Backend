...
test("community like toggle and reply comment flow", async ({ request }) => {
  ...
  try {
    const createPost = await api(request, "/api/community/posts", {
      ...
      token,
      body: {
        title: `playwright-like-reply-${Date.now()}`,
        content: "like toggle + reply regression",
        category: "FREE",
        tags: ["playwright", "reply"]
      }
    });
    ...
    const like = await api(request, `/api/community/posts/${postId}/reactions`, {
      method: "POST",
      token,
      body: { reaction: "LIKE" }
    });
    ...
    const detailAfterLike = await api(request, `/api/community/posts/${postId}`, { token });
    expect(detailAfterLike.status).toBe(200);
    expect(Number(detailAfterLike.body?.data?.likeCount ?? 0)).toBeGreaterThanOrEqual(1);

    const unlike = await api(request, `/api/community/posts/${postId}/reactions`, {
      method: "DELETE",
      token
    });
    expect(unlike.status).toBe(200);

    const detailAfterUnlike = await api(request, `/api/community/posts/${postId}`, { token });
    expect(detailAfterUnlike.status).toBe(200);
    expect(Number(detailAfterUnlike.body?.data?.likeCount ?? 0)).toBe(0);
    ...
    expect(Array.isArray(parentNode.replies)).toBe(true);
    expect(parentNode.replies.length).toBeGreaterThanOrEqual(1);
  } finally {
    if (postId) {
